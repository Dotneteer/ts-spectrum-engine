...
describe("Z80 CPU - IY-indexed 00-3f", () => {
  it("NOP", () => {
    // --- Arrange
  ...
  it("ADD IY,BC", () => {
    ...
    // --- Assert
    const s = m.cpu.getCpuState();
    expect(s.iy).toBe(0x2335);
    expect(s.cFlag).toBeFalsy();
    expect(s.hFlag).toBeFalsy();

    expect(s.nFlag).toBeFalsy();
    m.shouldKeepRegisters("IY, BC, F");
    m.shouldKeepMemory();
  ...
  it("LD (NN),IY", () => {
    ...
    // --- Assert
    s = m.cpu.getCpuState();
    expect(s.iy).toBe(0x1101);
    expect(m.memory[0x1000]).toBe(s.yl);
    expect(m.memory[0x1001]).toBe(s.yh);
  ...
  it("INC (IY+D)", () => {
    ...
    // --- Assert
    const s = m.cpu.getCpuState();
    expect(m.memory[s.iy + OFFS]).toBe(0xa6);
  ...
  it("DEC (IY+D)", () => {
    ...
    // --- Assert
    const s = m.cpu.getCpuState();
    expect(m.memory[s.iy + OFFS]).toBe(0xa4);
  ...
  it("LD (IY+D),N", () => {
    ...
    // --- Assert
    const s = m.cpu.getCpuState();
    expect(m.memory[s.iy + (256 - OFFS)]).toBe(0xa5);

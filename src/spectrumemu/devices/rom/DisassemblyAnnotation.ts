...
import { DisassemblyAnnotationData } from "./DisassemblyAnnotationData";
...
export class DisassemblyAnnotation {
     ...
     */
    readonly memoryMap: MemoryMap = new MemoryMap();
     ...
     * @returns True, if any modification has been done; otherwise, false
     */
    setLiteralReplacement(address: number, literalName: string | undefined): boolean {
     ...
     */
    applyLiteral(address: number, symbol: number, literalName: string | undefined): string | undefined {
        if (!literalName || !/\S/.test(literalName)) {
    ...
    toDisassemblyDecorationData(): DisassemblyAnnotationData {
        ...
        for (const section of this.memoryMap.sections) {
            dd.memorySections.push(new MemorySectionData(
                section.startAddress, section.endAddress, section.sectionType));
        }
    ...
    static deserialize(json: string): DisassemblyAnnotation | undefined {
        ...
        if (data) {
            ...
            for (const addr in data.prefixComments) {
                annotation.prefixComments.set(parseInt(addr), data.prefixComments[addr]);
            }
            ...
            for (const addr in data.literalReplacements) {
                annotation.literalReplacements.set(parseInt(addr), data.literalReplacements[addr]);
            }

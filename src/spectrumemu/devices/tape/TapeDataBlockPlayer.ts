...
export class TapeDataBlockPlayer implements ITapeData {
    ...
    initPlay(startTact: number): void {
        ...
        this._pilotEnds = ((this.data[0] & 0x80) === 0 ? this.headerPilotToneLength : this.dataPilotToneLength)
            * this.pilotPulseLength;
    ...
    getEarBit(currentTact: number): boolean {
        ...
        if (this.playPhase === PlayPhase.Pilot || this.playPhase === PlayPhase.Sync) {
            ...
            this._bitPulseLength = this._currentBit ? this.oneBitPulseLength : this.zeroBitPulseLength;
        }
        if (this.playPhase === PlayPhase.Data) {
            ...
            if (this.byteIndex < this.data.length) {
                ...
                this._bitPulseLength = this._currentBit 
                    ? this.oneBitPulseLength : this.zeroBitPulseLength;

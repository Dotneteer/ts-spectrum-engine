...
export interface Z80CpuTestSupport {
  ...
  setIff1(value: boolean): void;
  setIff2(value: boolean): void;
}
...
export function z80CpuEngine(
  ...
  /**
   * Defines the processing of extended instructions
   */
  const extendedOperations: Z80Operation[] = [
    // 0x00: ---
    null,
    ...
    () => {
      ...
      const value = memory.read(pc);
      tacts += 3;
      pc = (pc + 1) & 0xffff;
      const result = a & value;
      f = aluLogOpFlags[result] | FlagsSetMask.H;
      af = (a << 8) | f;
    },
    ...
    () => {
      ...
      wzh = wz >> 8;
      wzl = wz & 0xff;
      const pval = portInterface.readPort(bc);
      b = pval;
      bc = (b << 8) | c;
      f = aluLogOpFlags[pval] | (f & FlagsSetMask.C);
      af = (a << 8) | f;
    },
    ...
    () => {
      ...
      wzh = wz >> 8;
      wzl = wz & 0xff;
      const cfVal = (f & FlagsSetMask.C) !== 0 ? 1 : 0;
      let flags = FlagsSetMask.N;
      const reg = bc;
      flags |= (((hl & 0x0fff) - (reg & 0x0fff) - cfVal) >> 8) & FlagsSetMask.H;
      let sbcVal = (hl & 0xffff) - (reg & 0xffff) - cfVal;
      if ((sbcVal & 0x10000) !== 0) {
      ...
      var signedSbc = toSshort(hl) - toSshort(reg) - cfVal;
      ...
      h = hl >> 8;
      l = hl & 0xff;
      f = flags | (h & (FlagsSetMask.S | FlagsSetMask.R3 | FlagsSetMask.R5));
      af = (a << 8) | f;
      tacts += 7;
    },
    ...
    () => {
      ...
      const qq = bc;
      let flags =
        (((hl & 0x0fff) + (qq & 0x0fff) + (f & FlagsSetMask.C)) >> 8) &
        FlagsSetMask.H &
        0xff;
      const adcVal = (hl & 0xffff) + (qq & 0xffff) + cfVal;
      if ((adcVal & 0x10000) !== 0) {
        flags |= FlagsSetMask.C;
      }
      if ((adcVal & 0xffff) === 0) {
      ...
      const signedAdc = toSshort(hl) + toSshort(qq) + cfVal;
    ...
    // 0x4b: LD BC,(NNNN)
    () => {
      const addrl = memory.read(pc);
      tacts += 3;
      pc = (pc + 1) & 0xffff;
      const addr = (memory.read(pc) << 8) | addrl;
      tacts += 3;
      pc = (pc + 1) & 0xffff;
      wz = (addr + 1) & 0xffff;
      wzh = wz >> 8;
    ...
    () => {
      ...
      wzh = wz >> 1;
      wzl = wz & 0xff;
      memory.write(hl, ((a << 4) | (tmp >> 4)) & 0xff);
      tacts += 3;
    ...
    () => {
      ...
      wzh = wz >> 8;
      wzl = wz & 0xff;
      const low = memory.read(addr);
      tacts += 3;
      const high = memory.read(wz);
      tacts += 3;
      sp = (high << 8) | low;
    },
  ...
  const indexedOperations: (Z80Operation | number)[] = [
    ...
    // 0x84: ADD A,XH
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      f = adcFlags[(a << 8) + xVal];
      a = (a + xVal) & 0xff;
      af = (a << 8) | f;
    },
    ...
    // 0x85: ADD A,XL
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xl : yl;
      f = adcFlags[(a << 8) + xVal];
      a = (a + xVal) & 0xff;
      af = (a << 8) | f;
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[0](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[1](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[1](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[2](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[2](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[3](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[3](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[4](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[4](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[5](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[5](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[6](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[6](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
    ...
    () => {
      const xVal = indexMode === OpIndexMode.IX ? xh : yh;
      aluAlgorithms[7](xVal, (f & FlagsSetMask.C) !== 0);
    },
    ...
    () => {
      ...
      pc = (pc + 1) & 0xffff;
      const addr = (ixVal + toSbyte(offset)) & 0xffff;
      aluAlgorithms[7](memory.read(addr), (f & FlagsSetMask.C) !== 0);
      tacts += 3;
    },
  ...
  /**
   * RETN (RETI) instruction
   */
  function retn(): void {
    iff1 = iff2;
    let addr = memory.read(sp);
    tacts += 3;
    sp = (sp + 1) & 0xfff;
    addr += memory.read(sp) * 0x100;
    tacts += 3;
    sp = (sp + 1) & 0xfff;
    pc = addr;
    wz = addr;
  ...
  function processEDOperations(): void {
    const opMethod = extendedOperations[lastFethcedOpCode];
    if (opMethod) opMethod();
  }
